...
#treatments are not being considered as a factor and is numerical here so that is why it showing one huge boxplot. Seeding rate is being treated as numerical.
```
...
# we want to see this and fit a regresssion through
#this looks like quadratic will be the best 
```
...
#scaled residuals is to test for outliers. 
#left join to bring seeding rate back in table 
```
...
#interept is highly significant, but that does not mean you are running the best model.
```
...
#now we can see the line being fitted to the data... it is not great. Our model is underspecifing. 
```
